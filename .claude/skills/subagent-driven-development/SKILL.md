...
- 承認されるまで繰り返す
- 再レビューをスキップしない
- 静的解析(lint/pint)では検出できない指摘（命名規約の細かい好み、ライブラリの使い方など、プロジェクト固有で繰り返され得るもの）は `docs/review-history.json` に記録する。1回限りの実装ミスは記録しなくてよい。記録方法・閾値到達時の対応は **review-digest** スキル参照
- コード品質レビューを起動する際は必ず `REVIEW_ROUND` を指定する：そのタスクで最初の起動なら `first`（レビュアーが `npm run typecheck` / `composer stan` を実行する）、修正後の再レビューなら `re-review`（型チェックは再実行しない。90〜170秒かかる上、Windows/Docker Desktop のI/Oコストが支配的でスコープを絞っても速くならないため、再レビューのたびに重複実行するとタスク1つで数百秒の無駄が生じる）
...
**補助スキル：**
- **review-digest** — `docs/review-history.json` に蓄積された繰り返しレビュー指摘を、静的解析ルール等へ自動的に仕組み化する。全タスク完了後、または指摘の記録時に3回目に達したタイミングで起動を検討する
